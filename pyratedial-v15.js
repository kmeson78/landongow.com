...
let stationSwitchInFlight = false;
let scanInFlight = false;
let switchGeneration = 0;
let switchPollTimer = null;
let switchTimeoutTimer = null;
let switchOldPlaylistSignature = '';
let switchPlaylistConfirmed = false;
...
async function refreshStationMetadata(force = false) {
  ...
  if (!scanInFlight && !stationSwitchInFlight) renderStation(stations[currentIndex]);
}
...
function updateButtons() {
  const locked = scanInFlight || stationSwitchInFlight;
  prevButton.disabled = locked || currentIndex <= 0 || stations.length < 2;
  nextButton.disabled = locked || currentIndex >= stations.length - 1 || stations.length < 2;
}
...
function currentPlaylistSignature() {
  try {
    return playlistSignature(ytPlayer?.getPlaylist?.() || []);
  } catch (_) {
    return '';
  }
}

function finishStationSwitch() {
  if (!stationSwitchInFlight || !requestedStation) return;
  ...
  try {
    ...
    // Shuffle ONLY after the new playlist is confirmed. Calling it earlier can
    // reshuffle/advance the old station while YouTube is still switching.
    ytPlayer.setShuffle(true);
  } catch (_) {}

  revealPlayer();
  stationSwitchInFlight = false;
  switchPlaylistConfirmed = false;
  requestedStation = null;
  statusEl.textContent = 'SIGNAL LOCK';
  updateButtons();
}

function confirmPlaylistIfChanged(generation) {
  if (generation !== switchGeneration || !stationSwitchInFlight) return false;

  const signature = currentPlaylistSignature();
  if (!signature) return false;

  // On a station-to-station tune, the new playlist MUST differ from the old
  // playlist before we accept any PLAYING event as the new station.
  if (switchOldPlaylistSignature && signature === switchOldPlaylistSignature) {
    return false;
  }

  switchPlaylistConfirmed = true;
...
function beginVerifiedStationLoad(station, { fromPowerButton = false } = {}) {
  if (!powered || !playerReady || !ytPlayer || !station?.playlistId) return;

  clearSwitchTimers();

  requestedStation = station;
  stationSwitchInFlight = true;
  switchPlaylistConfirmed = false;
  switchGeneration += 1;
  const generation = switchGeneration;

  // POWER is a fresh start, so the first PLAYING event is trusted. AUTO TUNE
  // requires proof that YouTube's active playlist array actually changed.
  switchOldPlaylistSignature = fromPowerButton ? '' : currentPlaylistSignature();
  ...
  try {
    // Remove the old station immediately.
    ytPlayer.pauseVideo();
    ytPlayer.unMute();
    ytPlayer.setVolume(100);

    // Exactly ONE command changes stations. loadPlaylist also starts playback.
  ...
  if (fromPowerButton) {
    // The POWER tap is already known to work reliably on iPhone. We still keep
    // the monitor hidden until PLAYING fires.
    return;
  }

  // YouTube changes playlists asynchronously. Do not unlock AUTO TUNE merely
  // because the player says PLAYING; the old playlist can briefly emit that.
  // Poll getPlaylist() until its actual queue has changed.
  switchPollTimer = setInterval(() => {
    confirmPlaylistIfChanged(generation);
  }, 100);

  switchTimeoutTimer = setTimeout(() => {
    if (generation !== switchGeneration || !stationSwitchInFlight) return;
    clearSwitchTimers();
    stationSwitchInFlight = false;
    requestedStation = null;
    switchPlaylistConfirmed = false;
...
function powerOnReceiver() {
  ...
  powered = true;
  updatePowerControl();
  beginVerifiedStationLoad(stations[currentIndex], { fromPowerButton: true });
}

function powerOffReceiver() {
  powered = false;
  switchGeneration += 1; // invalidate any switch still completing in YouTube
  clearSwitchTimers();
  requestedStation = null;
  stationSwitchInFlight = false;
  switchPlaylistConfirmed = false;
...
function scanTo(targetIndex, userInitiated = false) {
  if (
    targetIndex < 0 ||
    targetIndex >= stations.length ||
    targetIndex === currentIndex ||
    scanInFlight ||
    stationSwitchInFlight
  ) return;

  const oldStation = stations[currentIndex];
  const targetStation = stations[targetIndex];

  // Change the LOGICAL station immediately. The old versions waited until the
  // visual dial animation ended, which meant a second AUTO press still used the
  // previous index and caused the one/two-station lag.
  currentIndex = targetIndex;
  localStorage.setItem('pyrateDial.lastFrequency', String(targetStation.frequency));

  scanInFlight = true;
  updateButtons();

  if (powered && playerReady && userInitiated) {
    beginVerifiedStationLoad(targetStation, { fromPowerButton: false });
  } else if (!powered) {
    setMonitorMessage('POWER OFF');
  }
  ...
  const startFrequency = oldStation.frequency;
  ...
  seekTimer = setInterval(() => {
    ...
    if (reached) {
      ...
      renderStation(
        targetStation,
        powered
          ? (stationSwitchInFlight ? 'TUNING' : 'SIGNAL LOCK')
          : 'POWER OFF'
      );
  ...
  ytPlayer = new YT.Player('youtubePlayer', {
    ...
    events: {
      ...
      onStateChange: event => {
        if (!powered) {
          if (event.data === YT.PlayerState.PLAYING) {
            try { event.target.pauseVideo(); event.target.mute(); } catch (_) {}
          }
          return;
        }

        if (stationSwitchInFlight) {
          // POWER: there is no competing prior load, so its first PLAYING event
          // is trustworthy and gives the exact behavior that already worked.
          if (!switchOldPlaylistSignature && event.data === YT.PlayerState.PLAYING) {
            finishStationSwitch();
            return;
          }

          // AUTO TUNE: verify the playlist array before trusting PLAYING.
          confirmPlaylistIfChanged(switchGeneration);

          if (event.data === YT.PlayerState.PLAYING) {
            if (switchPlaylistConfirmed) {
              finishStationSwitch();
            } else {
              // If the OLD playlist briefly resumes while the new list is still
              // loading, silence it instead of allowing a stray old-station song.
      ...
      onAutoplayBlocked: () => {
        ...
        switchPlaylistConfirmed = false;
        updateButtons();
        statusEl.textContent = powered ? 'SIGNAL HOLD' : 'POWER OFF';
      },

      onError: event => {
        // If the selected item is unavailable/private/unembeddable, stay on the
        // same station and let YouTube advance to the next playlist item.
        if ([100, 101, 150].includes(event.data) && powered) {
