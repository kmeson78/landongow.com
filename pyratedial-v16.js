...
function unlockStationSwitch() {
  ...
  try {
    // Loop persists across later playlists. Shuffle does not, so apply it only
    // after YouTube's queue has actually changed to the new station.
    ytPlayer.setLoop(true);
    ytPlayer.setShuffle(true);
  } catch (_) {}
}

function loadStation(station, { powerStart = false } = {}) {
  if (!powered || !playerReady || !ytPlayer || !station?.playlistId) return;

  clearSwitchTimers();
  stationSwitchInFlight = true;
  switchOldPlaylistSignature = powerStart ? '' : currentPlaylistSignature();
  ...
  try {
    // Stop the old station, then issue ONE playlist-load command. YouTube's
    // loadPlaylist() already loads AND plays; do not call playVideo() afterward.
    ytPlayer.pauseVideo();
    ytPlayer.unMute();
    ytPlayer.setVolume(100);

    // Argument syntax is deliberately used here to keep the station switch as
    // simple as possible inside the POWER/AUTO button's user gesture.
    ytPlayer.loadPlaylist(station.playlistId, startIndex, 0);
  } catch (error) {
  ...
  // POWER is already proven reliable on the phone. For AUTO TUNE, lock the
  // buttons until YouTube's actual playlist queue changes. Importantly, do NOT
  // pause any PLAYING event while waiting; that was what killed autoplay in v15.
  if (powerStart) return;

  switchPollTimer = setInterval(() => {
    const signature = currentPlaylistSignature();
    if (signature && signature !== switchOldPlaylistSignature) {
      unlockStationSwitch();
...
function powerOnReceiver() {
  ...
  powered = true;
  updatePowerControl();
  loadStation(stations[currentIndex], { powerStart: true });
}
...
function scanTo(targetIndex, userInitiated = false) {
  ...
  // Logical station changes immediately so another AUTO press can never be
  // calculated from the previous station.
  ...
  if (powered && playerReady && userInitiated) {
    loadStation(targetStation, { powerStart: false });
  } else if (!powered) {
    setMonitorMessage('POWER OFF');
  }
  ...
  ytPlayer = new YT.Player('youtubePlayer', {
    ...
    events: {
      ...
      onStateChange: event => {
        ...
        if (event.data === YT.PlayerState.PLAYING) {
          // Never pause a PLAYING event during a station change. YouTube may
          // report state changes before getPlaylist() has caught up, and pausing
          // here was the v15 autoplay failure.
          revealPlayer();
          ...
          // POWER start has no previous playlist to verify against, so its first
          // successful PLAYING event is enough to unlock the receiver.
          if (stationSwitchInFlight && !switchOldPlaylistSignature) {
            unlockStationSwitch();
          } else if (!stationSwitchInFlight) {
            statusEl.textContent = 'SIGNAL LOCK';
          }
        }

        if (event.data === YT.PlayerState.ENDED && powered) {
      ...
      onError: event => {
        // Removed/private/unembeddable item: stay on this station and advance.
