...
let stationSwitchSawTransition = false;
...
function updateButtons() {
  prevButton.disabled = stationSwitchInFlight || currentIndex <= 0 || stations.length < 2;
  nextButton.disabled = stationSwitchInFlight || currentIndex >= stations.length - 1 || stations.length < 2;
}
...
function startStation(station) {
  // Keep the existing iframe/player alive. A station change is one loadPlaylist()
  // call made directly from POWER or AUTO TUNE. loadPlaylist already loads AND
  // plays, so do not follow it with playVideo() — that was causing the previous
  // playlist to advance before the new playlist finished loading.
  if (!powered || !playerReady || !ytPlayer || !station?.playlistId) return;

  requestedStation = station;
  stationSwitchInFlight = true;
  stationSwitchSawTransition = false;
  updateButtons();
  ...
  try {
    // Silence/stop the old station immediately, but keep the same player.
    ytPlayer.pauseVideo();
    ytPlayer.unMute();
    ytPlayer.setVolume(100);

    // This function itself starts playback.
...
function scanTo(targetIndex, userInitiated = false) {
  ...
  // If the receiver is on, switch the actual playlist immediately while this
  // AUTO TUNE tap is still a user gesture. The visible dial animation follows.
  ...
  seekTimer = setInterval(() => {
    ...
    if (reached) {
      ...
      renderStation(targetStation, powered ? 'SIGNAL LOCK' : 'POWER OFF');
    ...
    events: {
      ...
      onStateChange: event => {
        if (event.data === YT.PlayerState.UNSTARTED || event.data === YT.PlayerState.BUFFERING) {
          if (stationSwitchInFlight) stationSwitchSawTransition = true;
          if (powered) statusEl.textContent = 'TUNING';
          return;
        }

        if (event.data === YT.PlayerState.PLAYING) {
          // Only declare a new station locked after the player has actually gone
          // through a load/buffer transition for the requested playlist.
          if (stationSwitchInFlight && !stationSwitchSawTransition) return;
